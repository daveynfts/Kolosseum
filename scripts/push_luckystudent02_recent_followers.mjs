/**
 * Merge @LuckyStudent02 recent followers into R2 recent-followers/v1.json.
 *
 *   node scripts/push_luckystudent02_recent_followers.mjs
 */
import fs from 'fs'
...
async function main() {
  ...
  map = { ...map, luckystudent02 }
  console.log('Set map.luckystudent02 =', luckystudent02.length)

  const payload = {
    version: 1,
    updatedAt: new Date().toISOString(),
    source: 'admin server · LuckyStudent02 recent followers curated',
    note: 'Update recent followers for @LuckyStudent02 (tuge8888)',
  ...
  const list = v.map?.luckystudent02
  console.log(
    'Verify luckystudent02:',
