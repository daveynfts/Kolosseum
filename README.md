...
The 25 September live test selected the top SCEX poster, @luong4101992 (36 posts), and saved a local [recorded walkthrough](./docs/DEMO_REPLAY.md). The Surf gateway rejects the saved key with HTTP 401 (UNAUTHORIZED); an anonymous Data API call can still return 200, so a report and paid end-to-end demo are not yet verified. Run npm run research:check-surf-auth before trying again; see the [Surf authentication diagnosis](./docs/SURF_AUTH.md). The /demo/replay page shows the real source and clearly marks unfinished stages; it makes no new Surf or pay.sh calls.
