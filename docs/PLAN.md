...
3. **M3 — trải nghiệm người mua:** wallet, `/me`, vote có bằng chứng mua, demo script và hướng dẫn 10 phút. Bán lại quyền đọc và split 80/20 chỉ mở sau khi có quyền thương mại/được phép bán lại Surf Output bằng văn bản (hoặc nguồn khác cho phép). Dừng cho review.
...
### Buyer dashboard, custom demo purchase, and votes (24 Sep 2026)

The feature-flagged `/me` route asks Phantom or Solflare to sign `Kolosseum buyer dashboard v1` plus wallet and ISO timestamp. The sidecar accepts that signature for two minutes on read-only `GET /me`; it cannot be reused as a `Kolosseum report access v1` signature. Queries filter `dr_reports.buyer_wallet`, `dr_channels.wallet`, and `dr_votes.wallet` and return at most the latest 100 records in each group, without report plaintext. Channel cap/spent/remaining in the UI are the last recorded verified claim, not a live on-chain balance. The CLI demo buyer owns reports with the CLI sandbox wallet; a browser wallet with a different address will correctly show no ownership.

The KOL panel now reads real `/health` and `/templates` responses, displays the configured $0.45 quick price and $1.00 deep authorization ceiling/$0.006 per Surf credit (minimum $0.10), and exposes a custom prompt. In private mode it retains admin-only generation; in gateway mode it shows the exact local sandbox demo command instead of a nonfunctional browser checkout button. The Windows script now accepts `-Prompt '<20–2000 character question>'` for the `/research/deep` x402 upto route, captures `PAYMENT-RESPONSE`, and claims that payment, while the template path still captures `Payment-Receipt` from an MPP session. Both paths reject missing Surf/PostgreSQL readiness before paying. The deep path passed PowerShell syntax and pre-payment refusal checks; an actual report purchase awaits those credentials.

`POST /reports/:id/vote` requires a fresh report-specific buyer signature and a report with a reconciled on-chain payment. It accepts only +1 or -1. A single SQL `INSERT ... SELECT` derives `weight_usdc` from `dr_reports.price_charged` and `proof_tx` from the uniquely claimed `payment_ref`, scoped to the matching buyer wallet; the client cannot supply weight or proof. The `proof_tx` column thus stores a prefixed settlement reference: `x402:<transaction>` or `mpp:<channel>:<cumulative voucher spend>`. Re-voting updates the existing row; public verify shows support/challenge counts and weighted sums. The receipt claim that precedes a vote has already checked sandbox-chain payer, amount, and settlement. Database-backed vote integration testing remains pending `DATABASE_URL`.

This does not complete M2/M3 acceptance: no real Surf-backed report has been bought, no browser wallet checkout exists, and resale is disabled by the Surf rights dependency.

### Local admin-only UI sandbox purchase (24 Sep 2026)

`PAY_DEMO_BUY_ENABLED=false` by default. On Windows, with a loopback-bound research sidecar, `PAY_MODE=sandbox`, and the gateway enabled, setting it to `true` exposes `POST /research/demo-buy` and `GET /research/demo-buy/latest` to requests bearing the existing `ADMIN_TOKEN`. The POST validates the X handle and one enabled template or a 20–2000 character non-trading prompt before spawning `pwsh -NoProfile -File scripts/research/demo-buyer.ps1` with argument-array execution (no shell interpolation). Missing Surf/database configuration returns 503 **before** the script or payment CLI starts. The sidecar permits one purchase at a time. The browser makes the user click a separate action that explicitly says the CLI sandbox wallet pays and Surf API usage may incur provider charges.

On success, the PowerShell script emits a structured `DEMO_RESULT_JSON:` line only after its existing receipt claim and devnet Memo check. The Node wrapper validates the report UUID, on-curve buyer wallet, MPP/x402 protocol, positive charge at or below $1, and devnet explorer URL before returning a report link. No raw receipt, Surf key, or provider output is returned to the browser. If a process error occurs after the report ID is known, the API returns `202 needs-review` with that ID and asks the admin to inspect it before paying again. A local in-memory latest-status endpoint helps recover from a browser timeout; its state is lost on sidecar restart.

The quick and deep CLI wire formats had previously passed isolated sandbox fixtures. The new UI route passed a real local preflight check: malformed handle returned 400 and missing Surf/database returned 503 with no CLI invocation. A complete paid Kolosseum UI purchase still awaits `SURF_API_KEY`, `DATABASE_URL`, migrations, and gateway signer configuration. It must not be marked end-to-end complete yet.

### One-command local sandbox demo (24 Sep 2026)

`npm run demo:start` now checks required Surf/PostgreSQL/payment configuration, sandbox mode, local gateway binding, and a 32-byte encryption key before doing any work. It requires the explicitly enabled UI demo flag, fixed loopback ports, and a local operator keypair. It applies the additive `dr_` migration, starts the research sidecar, waits for templates, starts pay.sh and checks its sandbox health, then starts Vite. Child processes are stopped when the launcher receives Ctrl+C or a service exits. On the current machine the real preflight exits before migration or payment because Surf, database, gateway signer, and paid-mode flags are not configured; no end-to-end report purchase is claimed.
