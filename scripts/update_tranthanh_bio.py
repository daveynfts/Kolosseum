#!/usr/bin/env python3
"""Write tranthanhbk bio into sheetKols + snapshot. Live = R2 via push_kol_bio."""
...
BIO = (ROOT / "data/bio-patches/tranthanhbk.txt").read_text(encoding="utf-8").strip()
HANDLE = "tranthanhbk"
FOLLOWERS = 116007
DISPLAY = "Trấn Thành"
...
def main() -> None:
    ...
    text = replace_ts_string_field(text, HANDLE, "displayName", DISPLAY)
    ...
    if snap_path.exists():
        ...
        for k in snap.get("kols", []):
            if str(k.get("handle", "")).lower() == HANDLE:
                k["bio"] = BIO
                k["followers"] = FOLLOWERS
                k["displayName"] = DISPLAY
                break
