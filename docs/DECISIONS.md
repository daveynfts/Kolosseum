...
## 2026-09-24 — Sandbox MPP session remains unverified

The official pay.sh bundled demo accepted one $0.01 sandbox payment through `pay --sandbox fetch`, proving the local CLI and hosted sandbox USDC path. The same client against Kolosseum's MPP session paywall opened a test channel but received HTTP 402 again and never reached the temporary capture origin. One sandbox USDC remains in that test channel escrow. This is a protocol integration blocker, not an application report purchase; do not claim M2 completion. The Windows `pay --sandbox curl` pass-through cannot locate curl despite an installed curl.exe, so local testing uses the documented built-in `pay fetch`. No mainnet wallet or funds were used.
