#!/usr/bin/env python3
"""Write Lecter bio into sheetKols + snapshot (fallback). Live display = R2 via push_kol_bio."""
...
BIO = (ROOT / "data/bio-patches/lecter_xfinance.txt").read_text(encoding="utf-8").strip()
FOLLOWERS = 42900
...
def replace_ts_string_field(text: str, handle: str, field: str, new_value: str | int) -> str:
    ...
    while j < len(text):
        ...
        raise SystemExit(f"unclosed string {field}")
...
def main() -> None:
    ...
    tmp = ROOT / "data/_lecter_report.txt"
    if tmp.exists():
        tmp.unlink()
