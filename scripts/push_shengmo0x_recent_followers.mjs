/**
 * Merge @ShengMo0x (ShengMo.eth) recent followers into R2 recent-followers/v1.json.
 *
 *   node scripts/push_shengmo0x_recent_followers.mjs
 */
import fs from 'fs'
...
async function main() {
  ...
  map = { ...map, shengmo0x }
  console.log('Set map.shengmo0x =', shengmo0x.length)

  const payload = {
    version: 1,
    updatedAt: new Date().toISOString(),
    source: 'admin server · ShengMo0x recent followers curated',
    note: 'Update recent followers for @ShengMo0x (allodev, bitgrateful, irenezhao_)',
