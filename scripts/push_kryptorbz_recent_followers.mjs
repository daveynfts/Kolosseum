/**
 * Merge @kryptorbz recent followers into R2 recent-followers/v1.json.
 *
 *   node scripts/push_kryptorbz_recent_followers.mjs
 */
import fs from 'fs'
...
async function main() {
  ...
  map = { ...map, kryptorbz }
  console.log('Set map.kryptorbz =', kryptorbz.length)

  const payload = {
    version: 1,
    updatedAt: new Date().toISOString(),
    source: 'admin server · kryptorbz recent followers curated',
    note: 'Update recent followers for @kryptorbz (4 accounts: jasonmdesimone … 466anan)',
