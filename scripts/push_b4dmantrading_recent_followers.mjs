/**
 * Merge @b4dmantrading recent followers into R2 recent-followers/v1.json.
 *
 *   node scripts/push_b4dmantrading_recent_followers.mjs
 */
import fs from 'fs'
...
async function main() {
  ...
  map = { ...map, b4dmantrading }
  console.log('Set map.b4dmantrading =', b4dmantrading.length)

  const payload = {
    version: 1,
    updatedAt: new Date().toISOString(),
    source: 'admin server · b4dmantrading recent followers curated',
    note: 'Update recent followers for @b4dmantrading (6 accounts: mikadontlouz … bitcoin136)',
  ...
  const list = v.map?.b4dmantrading
  console.log(
    'Verify b4dmantrading:',
