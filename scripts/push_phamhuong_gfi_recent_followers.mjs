/**
 * Merge @PhamHuong_GFI recent followers into R2 recent-followers/v1.json.
 *
...
async function main() {
  ...
  map = { ...map, phamhuong_gfi }
  console.log('Set map.phamhuong_gfi =', phamhuong_gfi.length)

  const payload = {
    version: 1,
    updatedAt: new Date().toISOString(),
    source: 'admin server · PhamHuong_GFI recent followers curated',
    note: 'Update recent followers for @PhamHuong_GFI (inkymaze)',
  ...
  const list = v.map?.phamhuong_gfi
  console.log(
    'Verify phamhuong_gfi:',
