/**
 * Merge @ZoeyCrypto0811 recent followers into R2 recent-followers/v1.json.
 *
 *   node scripts/push_zoeycrypto0811_recent_followers.mjs
 */
import fs from 'fs'
...
async function main() {
  ...
  map = { ...map, zoeycrypto0811 }
  console.log('Set map.zoeycrypto0811 =', zoeycrypto0811.length)

  const payload = {
    version: 1,
    updatedAt: new Date().toISOString(),
    source: 'admin server · ZoeyCrypto0811 recent followers curated',
    note: 'Update recent followers for @ZoeyCrypto0811 (x_may_crypto)',
  ...
  const list = v.map?.zoeycrypto0811
  console.log(
    'Verify zoeycrypto0811:',
