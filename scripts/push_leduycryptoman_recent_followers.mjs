/**
 * Merge @LeDuyCryptoMan recent followers into R2 recent-followers/v1.json.
 *
 *   node scripts/push_leduycryptoman_recent_followers.mjs
 */
import fs from 'fs'
...
async function main() {
  ...
  map = { ...map, leduycryptoman }
  console.log('Set map.leduycryptoman =', leduycryptoman.length)

  const payload = {
    version: 1,
    updatedAt: new Date().toISOString(),
    source: 'admin server · LeDuyCryptoMan recent followers curated',
    note: 'Update recent followers for @LeDuyCryptoMan (tokenmore, jasonmdesimone)',
  ...
  const list = v.map?.leduycryptoman
  console.log(
    'Verify leduycryptoman:',
