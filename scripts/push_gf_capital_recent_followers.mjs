/**
 * Merge @GF_Capital recent followers into R2 recent-followers/v1.json.
 *
 *   node scripts/push_gf_capital_recent_followers.mjs
 */
import fs from 'fs'
...
async function main() {
  ...
  map = { ...map, gf_capital }
  console.log('Set map.gf_capital =', gf_capital.length)

  const payload = {
    version: 1,
    updatedAt: new Date().toISOString(),
    source: 'admin server · GF_Capital recent followers curated',
    note: 'Update recent followers for @GF_Capital (ted55668, evans666666)',
  ...
  const list = v.map?.gf_capital
  console.log(
    'Verify gf_capital:',
    Array.isArray(list) ? list.length + ' recent followers' : 'MISSING',
  )
  if (Array.isArray(list)) {
    console.log(list.map((x) => `@${x.handle} (${x.followedAgo}) · ${x.displayName}`).join('\n'))
  }
}
