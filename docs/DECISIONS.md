...
## 25 Sep 2026 — Use the working Surf key and label fixed-tariff metering

The user replaced the rejected Surf key. Authenticated market-price Data and a real surf-2.0 Chat request succeeded, while the documented credit-balance route still returned 401 for the working key. The preflight therefore uses the authenticated Data route and reports its result as a Data authentication check, not a Chat guarantee. The Data call may consume standard credits; the gateway documents a short free-repeat window.

The completed Chat response included token counts but omitted the documented meta.credits_used. For new calls, store creditsSource provider when the value is present and published-rate when it is absent, using Surf's fixed credits-per-effort schedule. Cached calls store zero new credits with creditsSource cache. This value drives deep x402 metering and is shown with its provenance. At none effort the verified deep sandbox purchase charged 20 credits times $0.006, or $0.12. The gateway ceiling remains $1.00. The quick report price remains fixed at $0.45 even when its Surf response is reused from cache.

The first Surf draft miscounted the supplied posts as 21. A narrow deterministic correction now grounds that phrase to the actual context.posts length before encryption, hashing, or Memo submission. The first immutable preview row remains an audit record; the corrected report was stored as a new row and verified on devnet. All cited X status URLs in the paid quick and deep reports belong to the supplied post sample.

A Windows launcher issue stripped the Node/npm portion of PATH when adding Git-for-Windows utilities. The pay.sh launcher now preserves the inherited PATH. The MPP quick and x402 deep CLI purchases, receipt reconciliation, encrypted report storage, and devnet Memos completed with only sandbox USDC. Signed, encrypted captures for both purchase paths remain Git-ignored. The default UI replay shows MPP; the x402 capture is selectable by DEMO_REPLAY_FILE. Resale remains disabled pending commercial output rights.