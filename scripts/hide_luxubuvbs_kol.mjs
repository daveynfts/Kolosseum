/**
 * Hide @LuxubuVBS from public map (keep full record for later promote).
 * Reason: inactive / quiet on X for a long time.
 *
 *   node scripts/hide_luxubuvbs_kol.mjs
 *   node scripts/hide_luxubuvbs_kol.mjs --unhide
 */
import fs from 'fs'
...
async function main() {
  ...
  payload.note = unhide
    ? 'admin · re-promote @LuxubuVBS to public map'
    : 'admin · hide @LuxubuVBS from public map (inactive / quiet on X — kept for promote later)'
  payload.source = 'admin server · hide_luxubuvbs_kol'
