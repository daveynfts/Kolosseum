/**
 * Merge @ThanhCryptoBnb recent followers into R2 recent-followers/v1.json.
 *
 *   node scripts/push_thanhcryptobnb_recent_followers.mjs
 */
import fs from 'fs'
...
async function main() {
  ...
  map = { ...map, thanhcryptobnb }
  console.log('Set map.thanhcryptobnb =', thanhcryptobnb.length)

  const payload = {
    version: 1,
    updatedAt: new Date().toISOString(),
    source: 'admin server · ThanhCryptoBnb recent followers curated',
    note: 'Update recent followers for @ThanhCryptoBnb (3 accounts: jasonmdesimone … logearn_cn)',
  ...
  const list = v.map?.thanhcryptobnb
  console.log(
    'Verify thanhcryptobnb:',
