...
$payload = @{ kolHandle = $KolHandle; templateSlug = $TemplateSlug; buyerWallet = $buyerWallet } | ConvertTo-Json -Compress
$raw = & npx --yes @solana/pay@1.0.26 --sandbox --no-dna curl -i -sS -X POST -H 'Content-Type: application/json' -d $payload "$gatewayUrl/research/quick" 2>&1
...
$receiptLine = $raw | Where-Object { $_ -match '^(?i:payment-receipt):\s*' } | Select-Object -Last 1
if (-not $receiptLine) { throw 'Paid response contained no Payment-Receipt header.' }
...
Write-Host "Purchased report $($report.reportId) for @$KolHandle with sandbox MPP."

$claimBody = @{ protocol = 'mpp-session'; receipt = $receipt } | ConvertTo-Json -Compress
...
if (-not $claim) { throw 'MPP channel did not settle within the demo window; retry receipt claim later.' }
Write-Host "Payment verified: $($claim.priceChargedUsdc) sandbox USDC. Channel $($claim.channel.status), remaining $($claim.channel.remainingUsdc)."
